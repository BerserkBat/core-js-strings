...
function repeatString(str, times) {
  return times > 0 && typeof times === 'number' && Number.isFinite(times)
    ? str.repeat(times)
    : '';
}
...
function invertCase(str) {
  ...
  str.split('').forEach((char, index) => {
    if (char === strUpperCase.charAt(index)) {
      invertedStr.push(char.toLowerCase());
    } else {
      invertedStr.push(char.toUpperCase());
    }
  });
...
function encodeToRot13(str) {
  ...
  function newSymbol(char) {
    const index = input.indexOf(char);
    return index === -1 ? char : output.charAt(index);
  }

  str.split('').forEach((char) => {
    result += newSymbol(char);
  });
  return result;
}
